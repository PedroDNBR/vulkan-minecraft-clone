...
void VulkanRenderer::feedMesh(std::vector<Vertex> vertexData, std::vector<uint32_t> indicesData, std::vector<const char*> texturesPathsData)
{
...
void VulkanRenderer::destroyMeshAndFreeMeshSlot(int32_t index)
{
	if(index < 0) return;
	destroyMesh(*gpuMeshes[index]);
	gpuMeshes[index] = nullptr;
	freeMeshSlots.push_back(index);
}

void VulkanRenderer::queueMesh(int32_t i)
{
	if(i < 0) return;
	meshToDraw.push_back(i);
}

void VulkanRenderer::clearDrawMeshList()
{
	meshToDraw.clear();
}
...
void VulkanRenderer::initWindow()
{
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan Minecraft Clone", nullptr, nullptr);
...
void VulkanRenderer::createInstance()
{
	...
	appInfo.pApplicationName = "Vulkan Minecraft Clone";
	appInfo.applicationVersion = VK_MAKE_VERSION(0, 0, 1);
	appInfo.pEngineName = "Engine";
	appInfo.engineVersion = VK_MAKE_VERSION(0, 0, 1);
	appInfo.apiVersion = VK_API_VERSION_1_3; // if error, come back here and replace it with VK_API_VERSION_1_0
	...
	for (const auto& extension : requiredExtensions)
	{
		...
		if (!found)
		{
			throw std::runtime_error("Vulkan does not support extensions");
		}
	}*/
...
void VulkanRenderer::createSwapChain()
{
	...
	uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1; // it is recommended to request at least one more image than the minimum
	...
	if (indices.graphicsFamily != indices.presentFamily)
	{
		...
		std::cout << "graphics and presentation family are different" << std::endl;
	}
	else
	{
		...
		std::cout << "graphics and presentation family are the same" << std::endl;
	}
	...
	if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS)
		throw std::runtime_error("failde to create swap chain");
...
void VulkanRenderer::createFramebuffers()
{
	...
	for (size_t i = 0; i < swapChainImageViews.size(); i++)
	{
		...
		if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS)
			throw std::runtime_error("failed to create framebufffer");
	}
}
...
void VulkanRenderer::createCommandPool()
{
	...
	if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS)
		throw std::runtime_error("failed to crate command pool");
}
...
void VulkanRenderer::createTextureImage(uint32_t index)
{
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load(texturesPaths[index], &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void VulkanRenderer::createDescriptorsSets()
{
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
	...
	for (size_t i = 0; i < meshToDraw.size(); i++)
	{
		if (meshToDraw[i] >= gpuMeshes.size() ||
			gpuMeshes[meshToDraw[i]] == nullptr || 
			gpuMeshes[meshToDraw[i]]->indexCount < 1 ||
			gpuMeshes[meshToDraw[i]]->vertexBuffer == VK_NULL_HANDLE
		) 
			continue;

		VkDeviceSize offsets[] = { 0 };
		vkCmdBindVertexBuffers(commandBuffer, 0, 1, &gpuMeshes[meshToDraw[i]]->vertexBuffer, offsets);

		vkCmdBindIndexBuffer(commandBuffer, gpuMeshes[meshToDraw[i]]->indexBuffer, 0, VK_INDEX_TYPE_UINT32);


		vkCmdDrawIndexed(commandBuffer, gpuMeshes[meshToDraw[i]]->indexCount, 1, 0, 0, 0);
	}
...
uint32_t VulkanRenderer::uploadCpuMesh(const CpuMesh& cpuMesh)
{
	std::unique_ptr<GpuMesh> gpuMesh = std::make_unique<GpuMesh>();
	...
	if (freeMeshSlots.size() > 0)
	{
		uint32_t freeSlot = freeMeshSlots.back();
		freeMeshSlots.pop_back();
		gpuMeshes[freeSlot] = std::move(gpuMesh);
		return freeSlot;
	}

	gpuMeshes.push_back(std::move(gpuMesh));
	return gpuMeshes.size() - 1;
}
...
VkPresentModeKHR VulkanRenderer::chooseSwapChainPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
{
	...
	{
		if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) // Change to VK_PRESENT_MODE_IMMEDIATE_KHR to maximize framerate
...
uint32_t VulkanRenderer::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
	...
	throw std::runtime_error("failed top find suitable memory type");
}
...
{
	std::cerr << "validation layer - START\nseverity " << messageSeverity << "\nmessage: " << pCallbackData->pMessage << "\ntype: " << messagetype << "\nvalidation layer - END" << std::endl << std::endl;
